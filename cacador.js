      ...
      try {
        ...
        produtos = await page.evaluate(() => {
          ...
          function extrairSinaisQualidade(card) {
            ...
            return { nota, vendas, freteGratis, vendedorLider };
          }
          ...
          const brutos = cards.map((card, i) => {
            ...
            console.log(`[${i}] "${nome.slice(0,35)}" atual="${precoAtual}" orig="${precoOrig}" nota=${sinais.nota} vendas=${sinais.vendas} frete=${sinais.freteGratis} lider=${sinais.vendedorLider}`);
    ...
    const resultado = await Promise.all(
      produtos
        ...
        .map(async p => {
          ...
          console.log(`[Caçador] ML ✓ "${p.nome.slice(0,40)}" R$${atual}${desc > 0 ? ` (${desc}% OFF)` : ''}${selos ? ` [${selos}]` : ''}`);
          ...
          const idEstavel = idAnuncioDoLink(link);
          return {
...
async function buscarShopee() {
  ...
  try {
    ...
    const resultado = produtos.slice(0, 15).map(p => {
      ...
      return {
        id: `shopee-${p.itemid}`, produto: p.nome, loja: 'Shopee', categoria: 'Geral',
        precoAntigo: p.orig, precoAtual: p.atual, comissaoPct: 10, cupom: null, relampago: false,
        link, // link cru — o de afiliado é montado depois, por usuário
